...
enum
{
  ...
  GM_TABLE_ID = AT_TABLE_ID + 1, // Group Mapping Table
  NETIP_ID = GM_TABLE_ID + 1, // NETIP data
  CHECK_GA_DISPLAY = NETIP_ID + 1 , // ga display check
...
MyFrame::MyFrame(char* str_serial_number)
    : wxFrame(NULL, wxID_ANY, "KNX virtual IoT Router")
{
  ...
  Bind(wxEVT_MENU, &MyFrame::OnGroupMappingTable, this, GM_TABLE_ID);
  Bind(wxEVT_MENU, &MyFrame::OnNetIPData, this, NETIP_ID);
...
// Code for the KNX IOT ROUTER

/**
 * @brief shows the (loaded) group mapping table
 * 
 * @param event command triggered by a menu button
 */
void MyFrame::OnGroupMappingTable(wxCommandEvent& event)
{
  int device_index = 0;
  char text[1024 * 10];
  char line[500];
  bool ga_conversion = m_menuDisplay->IsChecked(CHECK_GA_DISPLAY);
  char windowtext[200];
  int max_entries = oc_core_get_group_mapping_table_size();
  int index = 1;
  ...
  strcpy(text, "");
  sprintf(line, "Max Group Mapping Entries '%d'\n", max_entries);
  strcat(text, line);

  for (index = 0; index < max_entries; index++) {

    oc_group_mapping_table_t* my_entry = oc_get_group_mapping_entry(device_index, index);
    if (my_entry != NULL) {
      if (my_entry->ga_len > 0) {
        sprintf(line, "index : '%d' id = '%d' \n", index, my_entry->id);
        strcat(text, line);
        sprintf(line, "  dataType : %d\n",my_entry->dataType);
        strcat(text, line);
        if (my_entry->ga_len > 0) {
          sprintf(line, "  osc_ga : [");
          strcat(text, line);
        ...
        if (oc_string_len(my_entry->groupKey) > 0) {
          sprintf(line, "  groupKey : ");
          strcat(text, line);
          int length = (int)oc_string_len(my_entry->groupKey);
          char* ms = oc_string(my_entry->groupKey);
          for (int i = 0; i < length; i++) {
            sprintf(line, "%02x", (unsigned char)ms[i]);
            strcat(text, line);
          }
  ...
  strcpy(windowtext, "Group Mapping Table ");
  strcat(windowtext, oc_string(device->serialnumber));
  CustomDialog(windowtext, text);
  SetStatusText("List group mapping entries");
}


/**
 * @brief shows the data of the NetIP communication
 * 
 * @param event command triggered by a menu button
 */
void MyFrame::OnNetIPData(wxCommandEvent& event)
{
  int device_index = 0;
  char text[1024 * 10];
  char line[500];
  ...
  strcat(windowtext, oc_string_checked(device->serialnumber));
  CustomDialog(windowtext, text);
  SetStatusText("List NetIP data");
}
...
{
  char text[500 + (200* 0)];
  strcpy(text, "KNX virtual IoT Router\n");
  ...
  strcat(text, "2023-07-31 10:49:07.716397");
